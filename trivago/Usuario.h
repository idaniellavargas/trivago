...
public:
	...
	Reserva(string titular, Hotel* h, string f, bool VIP, string correo, float ofer) {
		this->hotel = h;
		...
		this->correo = correo;
		if (VIP)monto = h->get_precioVIP() - ofer;
		else monto = h->get_precio() - ofer;
...
public:
	...
	void generardatos(int x, Catalogo* c) {
		for (int i = 0; i < x; i++) {
			if (i == 47) {
				cout << x;
			}
			reservas.push_back(new Reserva(generarnombres(), c->randH(), generarfecha(), (bool)stoi(generarbool()), generarcorreo(), 0));
		}
		cout << ".\n";
	}
};
...
public:
	...
	void reservarHotel(Catalogo* lista, Reservas* re, Cliente* c, Dueños* du, int ofer) {
		Hotel* h = new Hotel();
		string id;
		...
		if (h->get_precio() - ofer >= 0) cout << "El precio del hotel por persona es: " << h->get_precio() - ofer << endl;
		else cout << "El precio del hotel por persona es: " << 0 << endl;
		if (h->get_precioVIP() - ofer >= 0) cout << "El precio VIP es de: " << h->get_precioVIP() - ofer << endl;
		else cout << "El precio VIP es de: " << 0 << endl;
		...
		float costo;
		if (categoria == 1)costo = h->get_precio() - ofer;
		else costo = h->get_precioVIP() - ofer;
		cout << endl;
		float diff;
		diff = cartera - costo;
		if (diff >= 0)
		{
			Reserva* reserva = new Reserva(c->getnombre(), h, fecha->getDate(), 0, c->getcorreo(), ofer);
